...
- 资料面板实时呈现填表进度与执行诊断，简历数据完全隔离在扩展内部，杜绝网页脚本偷窥。
- 可选接入 OpenAI 兼容 API 或自定义 API，帮助理解不同网站的字段含义。
- AI 只辅助识别页面字段和可匹配的资料项，严格使用脱敏最小必要元数据与不透明字段序号，绝不发送简历具体值。
- 页面填写结果会用两种颜色标记：绿色已填写（严格读回校验），橙色待处理。
...
- **简历数据本机隔离**：简历资料严格保存在浏览器本地扩展存储中，设置页以独立标签页形式打开（`open_in_tab: true`）。
- **页面 DOM 零泄漏**：网页中的悬浮面板仅展示执行状态与进度，绝不将候选人姓名、手机号、工作经历等私密文本渲染进目标网页 DOM，防止宿主网站脚本窃取个人隐私。
- **AI 传输最小元数据边界**：AI 请求仅包含顶级域名（如 `zhaopin.com`）与会话级临时字段序号（如 `fld_1`、`fld_2`）、清洗后的标签与类型。**严禁传输完整 URL（不含 token 或查询参数）、严禁传输页面 title、严禁传输 DOM 路径、控件原生 id/name、环境文本及控件现有输入值**。
- **敏感控件绝对屏蔽**：密码框（`type="password"`）与文件上传（`type="file"`）被底层扫描器与写入器彻底物理屏蔽。
- **API 凭据安全**：API Key 仅保存在本机扩展存储，且导出简历备份时强制剔除 API 凭据。
- **终审权在用户**：插件不会自动点击最终提交按钮，页面自动填写后仍建议人工复核，尤其是证件、联系方式、日期、选择题和声明类字段。
